...
CONFIG_FIELDS: tuple[ConfigField, ...] = (
    ...
    ConfigField("interactive_commands", False, validator="bool", label="Interactive commands", section="runtime", desc="experimental: let the model type into commands waiting on stdin", ui_kind="bool", about="Experimental. When `true`, a command still alive once its output goes idle is held open and the model's next reply is typed into its stdin instead of shown as chat. When `false` (the default), every command runs to completion or is killed at `command_timeout`."),
