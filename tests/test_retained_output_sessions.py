...
import jarv.session_commands as session_commands
...
def test_reads_sidecar_follows_archive_restore_and_delete(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(session_commands, "SESSIONS_DIR", sessions_dir)
    monkeypatch.setattr(session_commands, "ARCHIVE_DIR", archive_dir)
    ...
    archived_history = session_commands.archive_session_files(history_path)
    ...
    restored_history = session_commands.unarchive_session_files(
        archived_history,
        "session-id",
    )
    ...
    session_commands.delete_session_files(restored_history)
